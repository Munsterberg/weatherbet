...
exports.generateSAID = (sa) => {
    ...
    if (saParam.name !== '_agreement' ||
        saParam.type !== 'tuple' ||
        !saParam.components) {
        throw Error(`extracted wrong version of struct tuple: ${saParam} from coordinatorFactory.interface.functions.getId`);
    }
    const abiValues = saParam.components.reduce((prev, next) => {
        prev.types.push(next.type);
        prev.values.push(sa[next.name]);
        return prev;
    }, { types: [], values: [] });
    return ethers_1.ethers.utils.solidityKeccak256(abiValues.types, abiValues.values);
};
